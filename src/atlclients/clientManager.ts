import { window, ConfigurationChangeEvent, ExtensionContext, Disposable, StatusBarItem, StatusBarAlignment } from 'vscode';
...
export class ClientManager implements Disposable {
    ...
    private async getClient<T>(provider:string, factory:(info:authinfo.AuthInfo)=>any):Promise<T | undefined> {
        ...
        if (!client) {
            ...
            if (!info) {
                ...
                await this._dancer.refresh(info).then(async(newInfo) => {
                    info = newInfo;
                    await Container.authManager.saveAuthInfo(provider,info);
                    await this.updateAuthenticationStatusBar(provider,info);
                }).catch(async() => {
    ...
    private async askUserToDance(provider:string): Promise<authinfo.AuthInfo | undefined> {
        const product = (provider === authinfo.AuthProvider.JiraCloud) ? 'Jira' : 'Bitbucket';
    ...
    private async getInLine(provider:string): Promise<authinfo.AuthInfo | undefined> {
    ...
    private async updateAuthenticationStatusBar(provider:string, info:authinfo.AuthInfo|undefined): Promise<void> {
		...
		if (statusBarItem) {
			...
			const newStatusBarItem = window.createStatusBarItem(StatusBarAlignment.Left);
    ...
    private async updateStatusBarItem(statusBarItem: StatusBarItem, provider:string, info:authinfo.AuthInfo|undefined): Promise<void> {
		let text: string;
