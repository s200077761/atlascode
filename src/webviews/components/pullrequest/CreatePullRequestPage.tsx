...
export default class CreatePullRequestPage extends WebviewComponent<Emit, Receive, {}, MyState> {
    ...
    handleBranchChange = () => {
        const sourceRemoteBranchName = this.state.sourceBranch
            ? this.state.sourceBranch.upstream &&
              this.state.sourceBranch.upstream.remote === this.state.sourceSiteRemote.remote.name
        ...
        if (
            ...
            this.state.sourceBranch !== this.state.destinationBranch
        ) {
            this.postMessage({
                action: 'updateDiff',
    ...
    handleJiraIssueStatusChange = (item: Transition) => {
        this.setState({
            ...
            issue: {
                ...(this.state.issue as MinimalIssue<DetailedSiteInfo>),
                status: {
            ...
            case 'createPullRequestData': {
                if (isCreatePRData(e)) {
                    ...
                    if (this.state.repo === emptyRepoData && e.repositories.length > 0) {
                        ...
                        this.resetRepoAndRemoteState(
                            firstRepo,
                            firstRepo.workspaceRepo.mainSiteRemote,
                            firstRepo.workspaceRepo.mainSiteRemote
                        );
                    }
                }
    ...
    diffPanelHeader = () => {
        return <h3>Files Changed {this.state.fileDiffsLoading ? '' : `(${this.state.fileDiffs.length})`}</h3>;
    };
    ...
    render() {
        ...
        if (repo.href) {
            externalUrl = repo.isCloud ? `${repo.href}/pull-requests/new` : `${repo.href}/pull-requests?create`;
        }
        const actionsContent = (
            <ButtonGroup>
                <Button className="ac-button" href={externalUrl}>
                    Create in browser...
                </Button>
            </ButtonGroup>
        );
                    ...
                    <div style={{ display: 'flex', alignItems: 'center' }}>
                        ...
                        {isMinimalIssue(this.state.issue) ? (
                            <div className="ac-flex">
                                ...
                                <NavItem
                                    text={`${this.state.issue.key} ${this.state.issue.summary}`}
                                    iconUrl={this.state.issue.issuetype.iconUrl}
                                    onItemClick={() =>
                                        this.postMessage({
                            ...
                            <div className="ac-flex">
                                ...
                                <NavItem
                                    text={`#${this.state.issue.data.id} ${this.state.issue.data.title}`}
                                    onItemClick={() =>
                                        this.postMessage({
                    ...
                    <GridColumn medium={6}>
                        ...
                        >
                            <div style={{ margin: 10 }}>
                                ...
                                {isMinimalIssue(this.state.issue) ? (
                                    <TransitionMenu
                                        transitions={(this.state.issue as MinimalIssue<DetailedSiteInfo>).transitions}
                                        currentStatus={(this.state.issue as MinimalIssue<DetailedSiteInfo>).status}
                                        isStatusButtonLoading={false}
                                        onStatusChange={this.handleJiraIssueStatusChange}
                                    />
                                ) : (
                                    <StatusMenu
                                        issueData={this.state.issue.data}
                                        isStatusButtonLoading={false}
        ...
        return (
            <div className="bitbucket-page">
                <Page>
                    <Form name="bitbucket-pullrequest-form" onSubmit={(e: any) => this.handleCreatePR(e)}>
                        {(frmArgs: any) => {
                            return (
                                    ...
                                    <Grid>
                                        ...
                                        {this.state.showPMF && (
                                            <PMFBBanner
                                                ...
                                                onPMFVisiblity={(visible: boolean) =>
                                                    this.setState({ showPMF: visible })
                                                }
                                        ...
                                        <GridColumn medium={6}>
                                            <div style={{ marginBottom: '20px' }}>
                                                ...
                                                <Select
                                                    ...
                                                    getOptionLabel={(repo: RepoData) =>
                                                        path.basename(repo.workspaceRepo.rootUri)
                                                    }
                                        ...
                                        <GridColumn medium={12}>
                                                        ...
                                                        <div className="ac-flex">
                                                            ...
                                                            <p style={{ marginLeft: '8px' }}>
                                                                {this.state.destinationSiteRemote
                                                                    ? `${
                                                                          this.state.destinationSiteRemote.site!
                                                                              .ownerSlug
                                                                      } / ${
                                                                          this.state.destinationSiteRemote.site!
                                                                              .repoSlug
                                                                      }`
                                                                    : 'No bitbucket remotes found'}
                                                            </p>
                                                        </div>
                                                        <div className="ac-compare-widget-break" />
                                                        {repo.workspaceRepo.siteRemotes.length > 1 && (
                                            ...
                                            <BranchWarning
                                                sourceBranch={
                                                    this.state.sourceBranch ? this.state.sourceBranch : undefined
                                                }
                                                sourceRemoteBranchName={this.state.sourceRemoteBranchName}
                                                remoteBranches={repo.remoteBranches}
                                                hasLocalChanges={repo.hasLocalChanges}
                                            />
                                            <CreatePRTitleSummary
                                                sourceBranchName={this.state.sourceBranch?.name ?? ''}
                                            ...
                                            <div className="ac-vpadding">
                                                <CheckboxField name="closeSourceBranch" id="closeSourceBranch">
                                                    {(fieldArgs: any) => {
                                                        return (
                                                            <Checkbox
                                                                {...fieldArgs.fieldProps}
                                        ...
                                        <GridColumn medium={12}>{issueDetails}</GridColumn>
                                        <GridColumn medium={12}>
                                            <div className="ac-vpadding">
                                                <Button
                                                    className="ac-button"
                                                    type="submit"
                                                    isLoading={this.state.isCreateButtonLoading}
                                                >
                                                    Create pull request
                                                </Button>
                                            </div>
