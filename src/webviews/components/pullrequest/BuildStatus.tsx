...
export default class BuildStatus extends React.Component<
    { buildStatuses?: BitbucketBuildStatus[]; postMessage: (e: OpenBuildStatusAction) => void },
    { dialogOpen: boolean }
> {
    ...
    render() {
        ...
        const resultIcon =
            inprogress.length > 0 ? (
                <Tooltip
                    content={`${inprogress.length} of ${this.props.buildStatuses.length} ${buildString} in progress`}
                    position="top"
                >
                    {inprogressIcon}
                </Tooltip>
                ...
                <Tooltip
                    content={`${successes.length} of ${this.props.buildStatuses.length} ${buildString} passed`}
                    position="top"
                >
                    {successIcon}
                </Tooltip>
                ...
                <Tooltip
                    content={`${this.props.buildStatuses.length - successes.length} of ${
                        this.props.buildStatuses.length
                    } ${buildString} unsuccessful`}
                    position="top"
                >
                    {errorIcon}
                </Tooltip>
                ...
                <InlineDialog
                    content={this.props.buildStatuses.map((status) => (
                        <Button
                            appearance="link"
                            onClick={() => {
                                this.props.postMessage({ action: 'openBuildStatus', buildStatusUri: status.url });
                            }}
                            iconBefore={
