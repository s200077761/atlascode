...
const headerSuccessIcon = (
    <CheckCircleIcon primaryColor={colors.N0} secondaryColor={colors.G400} label="build successful" />
);
const headerInprogressIcon = (
    <RecentIcon primaryColor={colors.N0} secondaryColor={colors.B300} label="build in progress" />
);
...
const headerErrorIcon = <ErrorIcon primaryColor={colors.N0} secondaryColor={colors.R400} label="build failure" />;
const calendarIcon = <CalendarIcon primaryColor={colors.N0} label="built at" />;
const builtTimeIcon = <RecentIcon primaryColor={colors.N0} label="build in progress" />;
...
export default class PipelineSummaryPage extends WebviewComponent<Emit, Pipeline, Properties, State> {
                    ...
                    return (
                        <div>
                            <span className="pipeline-step-title">
                                ...
                                <span>
                                    <span className="pipeline-step-head">{this.nameForStep(step, ix + 1)}</span>
    ...
    render() {
        ...
        return (
            <div>
                {!this.state.isOnline && this.state.pipeline.uuid === '' && <Offline />}
                                ...
                                breadcrumbs={
                                    <BreadcrumbsStateless onExpand={() => {}}>
                                        <BreadcrumbsItem
                                            component={() => (
                                                <NavItem
                                                    text={this.state.pipeline.repository!.name!}
                                                    href={this.state.pipeline.repository!.url}
                                                />
                                            )}
                                        />
                                        <BreadcrumbsItem
                                            component={() => (
                                                <NavItem
                                                    text="Pipelines"
                                                    href={`${this.state.pipeline.repository!.url}/addon/pipelines/home`}
                                                />
                                            )}
                                        />
                                        <BreadcrumbsItem
                            ...
                            >
                                ...
                                <span className="pipeline-head-item">
                                    ...
                                    {`${distanceInWordsToNow(
                                        this.state.pipeline.completed_on
                                            ? this.state.pipeline.completed_on
                                            : this.state.pipeline.created_on
                                    )} ago`}
                                </span>
                                <Avatar
                                    src={this.state.pipeline.creator_avatar}
                                    name={this.state.pipeline.creator_name}
                                    size="small"
                                />
                            </div>
                            {this.steps()}
