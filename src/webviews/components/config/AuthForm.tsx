...
> {
    ...
    setReadyToSave = (needsCreds: boolean) => {
        ...
        const readyToSave =
            FieldValidators.validateRequiredUrl(this.state.baseUrl, undefined) === undefined && credsAreGood;
    ...
    onBaseUrlChange = (e: any) => {
        if (FieldValidators.validateRequiredUrl(e.target.value, undefined) === undefined) {
            ...
            this.setState({ baseUrl: e.target.value, requiresCredentials: needsCreds }, () =>
    ...
    onUsernameChange = (e: any) => {
        this.setState({ username: e.target.value }, () => this.setReadyToSave(true));
    };

    onPasswordChange = (e: any) => {
        this.setState({ password: e.target.value }, () => this.setReadyToSave(true));
    };
    ...
    onSave = () => {
        ...
        const customSSLCerts =
            this.state.useCustomSSL && this.state.customSSLType === 'customServerSSL'
                ? this.state.certPaths
                : undefined;
        const pfxCert =
            this.state.useCustomSSL && this.state.customSSLType === 'customClientSSL' ? this.state.pfxPath : undefined;
        const pfxPassphrase =
            this.state.useCustomSSL && this.state.customSSLType === 'customClientSSL'
                ? this.state.pfxPassphrase
                : undefined;
    ...
    onCertPathsChange = (e: any) => {
        this.setState({ certPaths: e.target.value }, () => this.setReadyToSave(true));
    };

    onPfxPathChange = (e: any) => {
        this.setState({ pfxPath: e.target.value }, () => this.setReadyToSave(true));
    };

    onPfxPassphraseChange = (e: any) => {
        this.setState({ pfxPassphrase: e.target.value }, () => this.setReadyToSave(true));
    };
                ...
                >
                    ...
                    {this.state.requiresCredentials && (
                        <div>
                                ...
                                {(fieldArgs: any) => {
                                    return (
                                        <Checkbox
                                            ...
                                            onChange={FieldValidators.chain(
                                                fieldArgs.fieldProps.onChange,
                                                this.onContextPathEnableChange
                                            )}
                            ...
                            <CheckboxField name="custom-ssl-enabled" id="custom-ssl-enabled" value="custom.ssl.enabled">
                                {(fieldArgs: any) => {
                                    return (
                                        <Checkbox
                                            {...fieldArgs.fieldProps}
                            ...
                            {this.state.useCustomSSL && (
                                <Field
                                    defaultValue={this.state.customSSLType}
                                    label=""
                                    id="customSSLType"
                                    name="customSSLType"
                                >
                    ...
                    >
                        <div style={{ display: 'inline-flex', marginRight: '4px', marginLeft: '4px;' }}>
                            <Button className="ac-button" isDisabled={!this.state.readyToSave} onClick={this.onSave}>
                                Save
                            </Button>
                        </div>
                        <div style={{ display: 'inline-flex', marginRight: '4px', marginLeft: '4px;' }}>
