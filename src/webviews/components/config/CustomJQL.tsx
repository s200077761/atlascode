...
> {
    ...
    handleDrop = (e: any) => {
        ...
        if (
            this.state.dragSourceIndex !== undefined &&
    ...
    htmlForJQLEntry = (element: JQLEntry, displayIndex: number) => {
        return (
            ...
            >
                <div>
                    <Checkbox value={element.id} isChecked={element.enabled} onChange={this.toggleEnable} />
                </div>
    ...
    htmlElementAtIndex = (jql: JQLEntry[], index: number) => {
        ...
        return (
            <div id="jql-row-container" key={index} data-index={index}>
                <div
                    id="jql-row-drop-overlay"
                    data-index={index}
    ...
    render() {
        ...
        const noJql =
            jql.length === 0 ? (
                <p>
                    <em>No custom jql configured</em>
                </p>
            ) : (
                <React.Fragment />
            );
        const dragTip = (
            <p>
                <strong>Tip:</strong> You can drag/drop JQL entries in the list to change their display order in the
                explorer tree
            </p>
        );
