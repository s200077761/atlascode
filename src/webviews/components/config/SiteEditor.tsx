...
export const SiteEditor: React.FunctionComponent<AuthProps> = ({ sites, product, isRemote, handleDeleteSite, handleSaveSite, siteExample, cloudOrServer }) => {
    ...
    const generateLoginButtons = () => {
        if (cloudOrServer === undefined) {
            ...
            return <Button className="ac-button" isDisabled={isRemote} style={{ marginRight: '4px', display: 'block', width: '100%' }} onClick={handleCloudProd}>{loginText}</Button>;
        } else {
            return <Button className="ac-button" isDisabled={isRemote} style={{ marginRight: '4px', display: 'block', width: '100%' }} onClick={() => setAddingSite(true)}>{addSiteText}</Button>;
        }
    };
