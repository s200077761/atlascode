            ...
            <div style={{ padding: '2em' }}>
                <ButtonGroup>
                    ...
                    <Button
                        ...
                        onClick={() => this.postMessage({ action: 'openStartWorkPage' })}
                    >
                        Start work
                    </Button>
                ...
                {this.state.data.showJiraButton && (
                    <div className="ac-vpadding">
                        <Button className="ac-button" onClick={() => this.postMessage({ action: 'createJiraIssue' })}>
                            Create Jira Issue
                        </Button>
                    </div>
                )}
                <div className="ac-vpadding">
                    ...
                    <StatusMenu
                        issueData={issueData}
                        isStatusButtonLoading={this.state.isStatusButtonLoading}
                        onHandleStatusChange={(newStatus: string) => this.handleStatusChange(newStatus)}
                    />
                </div>
                <div className="ac-vpadding">
                    <label className="ac-field-label">Type</label>
                    <div className="ac-icon-with-text">
                        {typeIcon[issueData.kind!]}
                        <span style={{ paddingLeft: '1em' }}>{issueData.kind}</span>
                    </div>
                </div>
                <div className="ac-vpadding">
                    <label className="ac-field-label">Priority</label>
                    <div className="ac-icon-with-text">
                        {priorityIcon[issueData.priority!]}
                        <span style={{ paddingLeft: '1em' }}>{issueData.priority}</span>
                    </div>
                </div>
                <div className="ac-vpadding">
                    ...
                    <Tooltip content={issueData.reporter ? issueData.reporter.display_name : 'Unknown'}>
                        <AvatarItem
                            avatar={
                                <Avatar
                                    size="small"
                                    src={issueData.reporter ? issueData.reporter.links!.avatar!.href! : null}
                                />
                            }
                        ...
                        return (
                            <Grid>
                                <GridColumn medium={narrow ? 12 : 8}>
                                        ...
                                        breadcrumbs={
                                            <BreadcrumbsStateless onExpand={() => {}}>
                                                <BreadcrumbsItem
                                                    component={() => (
                                                        <NavItem
                                                            text={issueData.repository!.name!}
                                                            href={issueData.repository!.links!.html!.href}
                                                        />
                                                    )}
                                                />
                                                <BreadcrumbsItem
                                                    component={() => (
                                                        <NavItem
                                                            text="Issues"
                                                            href={`${issueData.repository!.links!.html!.href}/issues`}
                                                        />
                                                    )}
                                                />
                                                <BreadcrumbsItem
                                                    component={() => (
                                                        <NavItem
                                                            text={`Issue #${issueData.id}`}
                                                            href={issueData.links!.html!.href}
                                    ...
                                    >
                                        <Tooltip
                                            content={`Created on ${format(issueData.created_on, 'YYYY-MM-DD h:mm A')}`}
                                        >
                                            <React.Fragment>
                                                <p>{issueData.title}</p>
                                                <p
                                                    style={{ fontSize: 13, color: 'silver' }}
                                                >{`Created ${distanceInWordsToNow(issueData.created_on)} ago`}</p>
                                            </React.Fragment>
                                        </Tooltip>
                                    </PageHeader>
                                        ...
                                        {this.state.data!.hasMore && (
                                            <div className="ac-vpadding" style={{ textAlign: 'center' }}>
                                                <Button
                                                    appearance="subtle"
                                                    href={issueData.links!.html!.href}
                                                    iconAfter={<OpenIcon label="open-previous" />}
                                                >
