...
export async function checkout(pr: PullRequest, branch: string) {
  if (!pr.workspaceRepo) {
    Logger.error(
  ...
  const scm = Container.bitbucketContext.getRepositoryScm(
    pr.workspaceRepo.rootUri
  )!;

  if (
    pr.data.source.repo.url !== "" &&
    pr.data.source.repo.url !== pr.data.destination.repo.url
  ) {
    const parsed = parseGitUrl(
      urlForRemote(pr.workspaceRepo.mainSiteRemote.remote)
    );
    const sourceRemote = {
  ...
  scm
    ...
    .catch((e: any) => {
      Logger.error(
        new Error(`error checking out the pull request branch: ${e}`)
      );
    });
}
