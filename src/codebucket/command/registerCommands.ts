...
export function activate(context: ExtensionContext) {
  ...
  const openInBitbucketCmd = commands.registerCommand(
    CodebucketCommands.OpenInBitbucket,
    () => openInBitbucket.run()
  );
  context.subscriptions.push(openInBitbucketCmd);

  const openChangeset = new OpenBitbucketChangesetCommand();
  const openChangesetCmd = commands.registerCommand(
    CodebucketCommands.OpenChangeset,
    () => openChangeset.run()
  );
  context.subscriptions.push(openChangesetCmd);

  const openPullRequest = new OpenBitbucketPullRequestCommand();
  const openPullRequestCmd = commands.registerCommand(
    CodebucketCommands.ViewPullRequest,
    () => openPullRequest.run()
  );
  context.subscriptions.push(openPullRequestCmd);

  const copyPullRequest = new CopyBitbucketPullRequestCommand();
  const copyPullRequestCmd = commands.registerCommand(
    CodebucketCommands.CopyPullRequest,
    () => copyPullRequest.run()
  );
