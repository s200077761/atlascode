...
export function activate(context: ExtensionContext) {
    ...
    const openInBitbucketCmd = commands.registerCommand(CodebucketCommands.OpenInBitbucket, () => openInBitbucket.run());
    ...
    const openPullRequestCmd = commands.registerCommand(CodebucketCommands.ViewPullRequest, () => openPullRequest.run());
    context.subscriptions.push(openPullRequestCmd);

    const copyPullRequest = new CopyBitbucketPullRequestCommand();
    const copyPullRequestCmd = commands.registerCommand(CodebucketCommands.CopyPullRequest, () => copyPullRequest.run());
